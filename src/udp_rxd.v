...
module udp_rxd(
    ...
    (*mark_debug="true"*) output  reg          rxd_pkt_done,    //以太网单包数据接收完成信号
    (*mark_debug="true"*) output  reg          rxd_wr_en      ,    //以太网接收的数据使能信号
    (*mark_debug="true"*) output  reg  [31:0]  rxd_wr_data    ,    //以太网接收的数据
    (*mark_debug="true"*) output  reg  [15:0]  rxd_wr_byte_num ,    //以太网接收的有效字数 单位:byte     
    (*mark_debug="true"*) output  reg[31:0]      rxd_ip_rec,
    (*mark_debug="true"*) output  reg[31:0]      rxd_port_rec,
    (*mark_debug="true"*) output  reg            rxd_rec_ip_en
    );
...
(*mark_debug="true"*) reg          error_en        ; //解析错误使能信号
(*mark_debug="true"*) reg  [4:0]   cnt             ; //解析数据计数器
(*mark_debug="true"*) reg  [47:0]  destination_mac         ; //目的MAC地址
(*mark_debug="true"*) reg  [15:0]  eth_type        ; //以太网类型
(*mark_debug="true"*) reg  [31:0]  destination_ip          ; //目的IP地址
(*mark_debug="true"*) reg  [5:0]   ip_head_byte_num; //IP首部长度
(*mark_debug="true"*) reg  [15:0]  udp_byte_num    ; //UDP长度
(*mark_debug="true"*) reg  [15:0]  data_byte_num   ; //数据长度
(*mark_debug="true"*) reg  [15:0]  data_cnt        ; //有效数据计数    
(*mark_debug="true"*) reg  [1:0]   rxd_wr_en_cnt      ; //8bit转32bit计数器
...
///just for recieve pc ip and port  irene20240712

(*mark_debug="true"*)reg[7:0]       cnt_rxd_32_en;

(*mark_debug="true"*) wire            add_cnt_rxd_32_en;


//cnt_rxd_32_en will count more than 1+
always @(posedge clk or negedge rst_n) begin
    if(rst_n==1'b0)begin
        cnt_rxd_32_en<=0;
    end
    else if(cur_state!=state_rx_data)begin
        cnt_rxd_32_en<=0;
    end
    else if(add_cnt_rxd_32_en==1)begin
        cnt_rxd_32_en<=cnt_rxd_32_en+1;
    end
end

assign add_cnt_rxd_32_en= cur_state==state_rx_data&&rxd_wr_en==1;
...
always @(posedge clk or negedge rst_n) begin
    if(rst_n==1'b0)begin
      ...
      rxd_rec_ip_en<=0;
        ...
        if(cnt_rxd_32_en==2-1)begin
         rxd_ip_rec<=rxd_wr_data;
         rxd_rec_ip_en<=1;
        end
        else if(cnt_rxd_32_en==3-1)begin
         rxd_port_rec<=rxd_wr_data;
        end
    end
    else begin
        rxd_rec_ip_en<=0;
